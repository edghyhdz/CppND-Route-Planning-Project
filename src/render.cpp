...
void Render::Display( io2d::output_surface &surface )
{
    ...
    DrawSearchPoint(surface); 
    DrawPath(surface);
    // DrawPathSearched(surface); 
...
io2d::brush Render::TestFunction(RouteModel::Node current_node) const{
    
    float f = current_node.h_value + current_node.g_value; 
    float rgb_float = f * 255; 
    float rgb_float_2 = f * 200; 
    float rgb_float_3 = f * 125; 
    int rgb_int = (int) rgb_float; 
    int rgb_int_2 = (int) rgb_float_2; 
    int rgb_int_3 = (int) rgb_float_3; 

    io2d::brush foreBrush{ io2d::rgba_color{rgb_int_3, rgb_int_2, rgb_int}}; 
    return foreBrush; 
}

void Render::DrawPathSearched(io2d::output_surface &surface) const{
    io2d::render_props aliased{ io2d::antialias::none };
    io2d::brush foreBrush{ io2d::rgba_color::blue}; 
    float width = 5.0f;
    surface.stroke(foreBrush, PathLineSearched(), std::nullopt, io2d::stroke_props{width});
}

void Render::DrawSearchPoint(io2d::output_surface &surface) const{
    if (m_Model.searched_nodes.empty()) return;
    for ( auto k : m_Model.searched_nodes ){
        io2d::render_props aliased{ io2d::antialias::none };
        io2d::brush foreBrush = TestFunction(k);
        auto pb = io2d::path_builder{}; 
        pb.matrix(m_Matrix);
        pb.new_figure({(float) k.x, (float) k.y});
        float constexpr l_marker = 0.015f;
...
/*
Extra attribute definition to color up searched path
*/
io2d::interpreted_path Render::PathLineSearched() const
{    
    if( m_Model.searched_nodes.empty() )
        return {};

    const auto nodes = m_Model.searched_nodes;    
    
    auto pb = io2d::path_builder{};
    pb.matrix(m_Matrix);
    pb.new_figure( ToPoint2D( m_Model.searched_nodes[0]));

    for( int i=1; i< m_Model.searched_nodes.size();i++ )
        pb.line( ToPoint2D(m_Model.searched_nodes[i])); 
